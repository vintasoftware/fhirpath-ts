...
describe('cachedCompile', () => {
  ...
  it('does not retain expressions when capacity is zero', () => {
    const cache = createParseCache(0)
    const first = cachedCompile('Patient.active', cache)
    expect(cachedCompile('Patient.active', cache)).not.toBe(first)
  })

  it('rejects invalid capacities at the API boundary', () => {
    expect(() => createParseCache(-1)).toThrow('non-negative integer')
    expect(() => createParseCache(1.5)).toThrow('non-negative integer')
  })
