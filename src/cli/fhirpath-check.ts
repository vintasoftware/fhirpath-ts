 ...
 * analyzer over each with the R4 model. Exits non-zero when any diagnostic is
 * found. It is the standalone equivalent of the ./eslint rule, for repos that do
 * not lint with ESLint (e.g. Biome repos). This repo itself dogfoods the ./eslint
 * rule, so its own CI enforces the checks through `pnpm lint`.
 *
