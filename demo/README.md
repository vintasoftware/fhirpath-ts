...
## The playground's declarations

The Monaco playground type-checks the reader's code against the library's real
declarations, which `scripts/generate-dts.mjs` rolls up into `src/monaco/*.d.ts`.
Those are build artifacts — gitignored, and regenerated by `npm run dev` and
`npm run build` before Vite starts, so they cannot fall behind `../src`. Run
`npm run generate:dts` on its own if you want them refreshed without a build.
