...
export const ANALYZER_SKIP_MANIFEST: SkipEntry[] = [
  ...
  },
  {
    ...
    reason: 'the iif() criterion is not yet checked for cardinality (WS2 analyzer upgrades)',
  },
  {
    suite: 'r5',
    group: 'testCollectionBoolean',
  ...
  // r4 testIif6 needs no entry: its runtime skip (R5 revised the semantics) already excludes it here.
  {
    ...
    group: 'testIif',
    test: 'testIif6',
    reason: 'the iif() criterion is not yet checked for Boolean-ness (WS2 analyzer upgrades)',
  },
  {
    suite: 'r5',
    group: 'defineVariable',
    test: 'defineVariable9',
    reason: 'defineVariable() scope and redefinition are not yet tracked statically (WS2 analyzer upgrades)',
  },
  {
    suite: 'r5',
    group: 'defineVariable',
